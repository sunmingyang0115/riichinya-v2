...
export class MessageCreateHandler implements EventBuilder {
    ...
    getEventCallFunction(m : Message) {
        ...
        if (frag[0] == "ron" && this.messageMap.has(frag[1])) {
            ...
            let cb = this.messageMap.get(frag[1]);
            let builder : string[] = [
                "```ini",
                `#!/nya/docs/${cb?.getCommandName()}`,
                "",
                "[Properties]",
                `Command_Name = ${cb?.getCommandName()}`,
                `Aliases = []`,
                `Cooldown = ${cb?.getCooldown()}`,
                "",
                "[Usage]",
                `${this.messageMap.get(frag[1])!.getDocumentation().build()}`,
                "```"
            ]
            m.reply(builder.join("\n"));
        }
